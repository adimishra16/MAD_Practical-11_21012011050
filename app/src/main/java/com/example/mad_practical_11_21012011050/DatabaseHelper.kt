...
class DatabaseHelper(context: Context?):
    ...
    companion object {
        ...
        private const val DATABASE_NAME = "person_db"
    }
    ...
    override fun onUpgrade(db: SQLiteDatabase, otdVersion: Int, newVersion: Int) {

        db.execSQL("DROP TABLE EXISTS" + PersonTable.TABLE_NAME)
    }
    ...
    private fun getValues(person: Person): ContentValues {
        ...
        values.put(PersonTable.COLUMN_EMAIL, person.emailid)
        values.put(PersonTable.COLUMN_PHONENO, person.phoneNo)
        values.put(PersonTable.COLUMN_LAT, person.latitude)
        values.put(PersonTable.COLUMN_ADDRESS, person.address)
        values.put(PersonTable.COLUMN_LONG, person.longitude)
        return values
    }
    ...
    fun getPerson(id: String): Person? {
        ...
        val cursor = db.query(
            ...
            arrayOf(
                ...
                PersonTable.COLUMN_EMAIL,
                PersonTable.COLUMN_PHONENO,
                PersonTable.COLUMN_ADDRESS,
                PersonTable.COLUMN_LAT,
                PersonTable.COLUMN_LONG
            ),
            PersonTable.COLUMN_ID + "=?",
            arrayOf(id),
            null,
    ...
    private fun getPerson(cursor: Cursor): Person {
        return Person(
            ...
            cursor.getString(cursor.getColumnIndexOrThrow(PersonTable.COLUMN_EMAIL)),
            ...
            cursor.getDouble(cursor.getColumnIndexOrThrow(PersonTable.COLUMN_LAT)),
            cursor.getDouble(cursor.getColumnIndexOrThrow(PersonTable.COLUMN_LONG))
        )
    }
